...
float RealSuccessRatio = 0.0f;
#endif
size_t NReps = 1;

void escribirCSV(std::string nombreArchivo, int num_iterations, double execution_time) {
    
    #ifdef SLSIMULATE
      if(SimulatedSuccessRatio != -1) {
        nombreArchivo = "simulated_success/" + nombreArchivo;
      } else {
        nombreArchivo = "non_simulated_success/" + nombreArchivo;
      }
    #else
        nombreArchivo = "non_simulated_success/" + nombreArchivo;
    #endif

    std::ofstream archivo(nombreArchivo, std::ios_base::app); // Abrir en modo añadido
    ...
    if (archivo.tellp() == 0) {
        archivo << "num_chunks,min_threads,num_iterations";
        
        #ifdef SLSIMULATE
        if(SimulatedSuccessRatio != -1) {
          archivo << ",simulated_success_ratio";
        }
        #endif  
        archivo
        ...
        << ",num_repetitions,num_threads,execution_time\n";
    }
...
void escribirCSV(std::string nombreArchivo, int num_iterations, int cache_faults, double execution_time) {
    ...
    if (archivo.tellp() == 0) {
        ...
        << ",num_repetitions,num_threads,cache_faults,execution_time\n";
    }
...
void escribirCSV(std::string nombreArchivo, int num_iterations, double execution_time, float result) {
    ...
    if (archivo.tellp() == 0) {
        ...
        << ",num_repetitions,num_threads,tolerance,float_size,result,execution_time\n";
    }
...
void escribirCSV(std::string nombreArchivo, int num_iterations, double execution_time, float result, int tol2) {
    ...
    if (archivo.tellp() == 0) {
        ...
        << ",num_repetitions,num_threads,tolerance,tolerance2,float_size,result,execution_time\n";
    }
  ...
  while( (c = optClass.getopt(argc, argv, optstr)) != -1 ) {
    switch (c) {
      ...
      case 'i':
        ...
        break;
      case 'l':
        FloatSize = (size_t) strtoul(optarg, nullptr, 0);
        break;
      case 'o':
        Tolerance = (float) strtof(optarg, nullptr);
        break;
      case 'z':
        Tolerance2 = (float) strtof(optarg, nullptr);
        break;
#ifdef THREADINSTRUMENT
      case 'v':
        Verbose = true;
...
void printStatsRunInfo(const SpecLib::StatsRunInfo& statsRes, const int n = -1) {
  ...
  if (statsRes.total > 0) {
    ...
    RealSuccessRatio = static_cast<double>(statsRes.successes)*100.0/static_cast<double>(statsRes.total);
